...
// (consistent with the memstream and snapstore packages): the only thing needed
...
//
// Auth uses the agent VM's instance service-account token from the GCE metadata
// server, cached and refreshed shortly before expiry — the same identity the
// full-download path uses today.
...
// maxGCSAttempts bounds transient-error retries for a single chunk fetch. A
// cold first-fault Range-GET can hit a momentary 5xx / connection reset; without
// a retry that surfaces as an NBD EIO and (eventually) a watchdog recovery, far
// more disruptive than a short backoff. Bounded so a persistently-down GCS still
// fails fast into the never-fail fallback rather than hanging.
const maxGCSAttempts = 4
...
func (s *gcsRangeSource) ReadAt(ctx context.Context, p []byte, off int64) (int, error) {
	...
	for attempt := 0; attempt < maxGCSAttempts; attempt++ {
		if attempt > 0 {
			fireGCSRetry()
		...
		n, err := s.readOnce(ctx, p, off)
		if err == nil {
			return n, nil
		}
		// Context cancellation/deadline is not transient — don't retry it.
		if ctx.Err() != nil {
			return n, err
		}
		lastErr = err
	}
	return 0, fmt.Errorf("diskstream: GCS fetch failed after %d attempts: %w", maxGCSAttempts, lastErr)
}

func (s *gcsRangeSource) readOnce(ctx context.Context, p []byte, off int64) (int, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("diskstream: get token: %w", err)
	}
	...
	if resp.StatusCode != http.StatusPartialContent && resp.StatusCode != http.StatusOK {
		return 0, fmt.Errorf("diskstream: GET %s range %d-%d: %s", s.object, off, end, resp.Status)
	}
...
func (m *metadataTokenProvider) Token(ctx context.Context) (string, error) {
	...
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("diskstream: metadata token: %s", resp.Status)
	}
	var body struct {
	...
	if body.AccessToken == "" {
		return "", fmt.Errorf("diskstream: empty access token")
	}

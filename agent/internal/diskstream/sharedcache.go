...
// SharedCache is a persistent, per-seed-generation rootfs-chunk store shared by
// every streaming restore of the same disk object on a host. It sits between the
// per-device Resolver and the remote ChunkSource (GCS):
//
//	guest read → NBD server → Resolver (per-device) → SharedCache (per-template,
...
// chunks. With it, only the first restore of a seed generation on a host touches
// the network; every later restore (including concurrent ones) is served from
// local NVMe / page cache. This is what makes the working-set numbers from Phase
// 0c (62 MiB boot, ~223 MiB with a runtime) a once-per-host cost rather than a
// per-restore cost, and why a Node runtime fetched for one app serves every Node
// app on the host.
//
...
//
// Crash safety (inherited from the memstream design unchanged): the bitmap is
// advanced only by (1) fdatasync(chunks.dat), then (2) atomic tmp+rename of
// present.psc containing a snapshot of the bits taken BEFORE the sync. Any chunk
// the bitmap claims valid had its data durable first — a crash between the steps
// merely forgets recent chunks (re-fetched next time), never serves torn data.
//
// Integrity: because the SharedCache is the boundary at which user-controlled
// rootfs bytes cross from GCS into the host, it verifies each fetched chunk
// against the bake-time SHA-256 in the Header before writing it. A mismatch is a
// hard fetch error — corrupt blocks never reach chunks.dat, so a bitmap-valid
// chunk is both durable AND integrity-checked.
//
// The cache key is content-addressed (bucket/object of the seed generation), so
// a re-baked template gets a fresh directory rather than a stale overlay.
...
const (
	...
	// SharedBitmapMagic identifies a v1 disk shared-cache presence bitmap
	// ("PandaStack Disk Cache v1"). Distinct from the memstream cache magic
	// ("PSC1") and the diskstream header/prefetch magics so a misplaced file is
	// rejected, not misparsed.
	SharedBitmapMagic = "PSCD"
...
// OpenSharedCache opens (or creates) the shared chunk cache in dir for the rootfs
// object described by h, backed by upstream for cache misses. The cache takes
// ownership of upstream: SharedCache.Close closes it.
//
// A geometry mismatch with an existing cache (different TotalSize) or an
// unreadable bitmap resets the cache to empty rather than failing: worst case is
// re-fetching, never serving wrong bytes.
func OpenSharedCache(dir string, h *Header, upstream ChunkSource) (*SharedCache, error) {
	if h == nil {
		return nil, fmt.Errorf("diskstream: shared cache: nil header")
	}
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return nil, fmt.Errorf("diskstream: shared cache mkdir: %w", err)
	}
	dataPath := filepath.Join(dir, sharedDataFile)
	f, err := os.OpenFile(dataPath, os.O_RDWR|os.O_CREATE, 0o644)
	if err != nil {
		return nil, fmt.Errorf("diskstream: shared cache open data: %w", err)
	}
	...
	if fresh {
		...
		if err := f.Truncate(0); err != nil {
			_ = f.Close()
			return nil, fmt.Errorf("diskstream: shared cache reset data: %w", err)
		}
		if err := f.Truncate(int64(h.TotalSize)); err != nil {
			_ = f.Close()
			return nil, fmt.Errorf("diskstream: shared cache size data: %w", err)
		}
	}
...
func (c *SharedCache) ReadAt(ctx context.Context, p []byte, off int64) (int, error) {
	...
	if off < 0 || off >= int64(c.h.TotalSize) {
		return 0, fmt.Errorf("diskstream: shared cache: offset %d out of range (size %d)", off, c.h.TotalSize)
	}
	end := off + int64(len(p))
	...
	for i := first; i <= last; i++ {
		// Absent (all-zero) chunks need no fetch: the sparse data file reads
		// back zeros for untouched regions by construction.
		if !c.h.IsPresent(i) {
			continue
		}
...
func (c *SharedCache) ensureChunk(ctx context.Context, idx int) error {
	...
	if idx < 0 || idx >= len(c.present) {
		c.mu.Unlock()
		return fmt.Errorf("diskstream: shared cache: chunk %d out of range", idx)
	}
	if c.present[idx] {
		c.mu.Unlock()
		c.hits.Add(1)
...
func (c *SharedCache) fetchChunk(ctx context.Context, idx int) error {
	...
	if err != nil {
		return fmt.Errorf("diskstream: shared cache fetch chunk %d (off %d len %d): %w", idx, off, length, err)
	}
	if int64(n) < length {
		return fmt.Errorf("diskstream: shared cache short fetch chunk %d: got %d want %d", idx, n, length)
	}
	// Integrity at the GCS boundary: verify before persisting so a bitmap-valid
	// chunk is always both durable and integrity-checked.
	if want, ok := c.h.ChunkSHA(idx); ok {
		if got := sha256.Sum256(buf[:n]); got != want {
			c.verifyErr.Add(1)
			fireVerifyError()
			return fmt.Errorf("diskstream: shared cache chunk %d integrity check failed (off %d len %d)", idx, off, length)
		}
	}
	if _, err := c.data.WriteAt(buf[:n], off); err != nil {
		return fmt.Errorf("diskstream: shared cache write chunk %d: %w", idx, err)
	}
	c.fills.Add(1)
	return nil
}
...
func (c *SharedCache) Flush() error {
	...
	if err := c.data.Sync(); err != nil {
		return fmt.Errorf("diskstream: shared cache fsync: %w", err)
	}
...
// Stats reports (local hits, upstream fills, integrity rejects) since open.
func (c *SharedCache) Stats() (hits, fills, verifyErr int64) {
	return c.hits.Load(), c.fills.Load(), c.verifyErr.Load()
}

// Close stops the flusher, performs a final flush, and closes the data file and
// the upstream source. Idempotent.
...
//
// chunkSize/totalSize are recorded so a bitmap from a different geometry is
// rejected instead of marking wrong chunks valid.
...
func writeSharedBitmap(path string, h *Header, present []bool) error {
	...
	if err := os.WriteFile(tmp, encodeSharedBitmap(h, present), 0o644); err != nil {
		return fmt.Errorf("diskstream: shared cache bitmap write: %w", err)
	}
	if err := os.Rename(tmp, path); err != nil {
		_ = os.Remove(tmp)
		return fmt.Errorf("diskstream: shared cache bitmap rename: %w", err)
	}
	return nil
}
...
func readSharedBitmap(path string, h *Header) ([]bool, error) {
	...
	if len(b) < fixed {
		return nil, fmt.Errorf("diskstream: shared bitmap too short (%d bytes)", len(b))
	}
	if string(b[:4]) != SharedBitmapMagic {
		return nil, fmt.Errorf("diskstream: bad shared bitmap magic %q", b[:4])
	}
	off := 4
	ver := binary.LittleEndian.Uint32(b[off:])
	off += 4
	if ver != sharedBitmapVersion {
		return nil, fmt.Errorf("diskstream: unsupported shared bitmap version %d", ver)
	}
	...
	if chunkSize != h.ChunkSize || totalSize != h.TotalSize || n != h.NumChunks() {
		return nil, fmt.Errorf("diskstream: shared bitmap geometry mismatch (chunk %d/%d total %d/%d chunks %d/%d)",
			chunkSize, h.ChunkSize, totalSize, h.TotalSize, n, h.NumChunks())
	}
	bitmapLen := (n + 7) / 8
	if len(b)-off < bitmapLen {
		return nil, fmt.Errorf("diskstream: truncated shared bitmap (need %d, have %d)", bitmapLen, len(b)-off)
	}
...
// SharedCacheKey derives the content address for a rootfs object: the seed
// publisher writes each generation to a unique bucket/object path, so hashing it
// yields a stable per-generation key (a re-bake = new object = new key). It is
// prefixed "disk-" so it can never collide with a memstream key sharing a root.
func SharedCacheKey(bucket, object string) string {
	sum := sha256.Sum256([]byte("disk\x00" + bucket + "/" + object))
	return "disk-" + hex.EncodeToString(sum[:8])
}
...
// evicted in the background until under budget. The disk cache uses its own
// budget knob (PANDASTACK_DISK_CACHE_MAX_GB) since rootfs working sets and image
// counts differ from memory; both caches may share a root but evict
// independently because their keys are prefixed disjointly ("disk-" here).
func AcquireSharedCache(root, key string, h *Header, maxBytes int64, mk func() (ChunkSource, error)) (ChunkSource, error) {
	...
	if c, ok := sharedReg.m[key]; ok {
		if c.h.TotalSize != h.TotalSize || c.h.ChunkSize != h.ChunkSize {
			return nil, fmt.Errorf("diskstream: shared cache %s geometry changed mid-process", key)
		}
...
// sharedRef is the per-restore handle on a process-wide SharedCache. Close is a
// no-op: the cache outlives every individual restore by design.
...
// EnsureChunk warms a chunk into the underlying shared cache (so a prefetch
// against the ref benefits every restore on the host).
func (r *sharedRef) EnsureChunk(ctx context.Context, idx int) error { return r.c.EnsureChunk(ctx, idx) }

// Stats exposes the underlying cache counters (hits, fills, verify errors).
func (r *sharedRef) Stats() (int64, int64, int64) { return r.c.Stats() }

// evictSharedLRU removes least-recently-used disk cache directories under root
// until total allocated bytes fit maxBytes. "Recently used" is the bitmap mtime
// (advanced on every flush of new fetches). Only "disk-"-prefixed dirs are
// considered, and live (open) caches are never evicted. Best-effort: errors are
// swallowed — eviction is hygiene, not correctness.
func evictSharedLRU(root string, live map[string]bool, maxBytes int64) {
	...
	for _, e := range entries {
		if !e.IsDir() || len(e.Name()) < 5 || e.Name()[:5] != "disk-" {
			continue
		}
...
// allocation), so sparse cache files are accounted by fetched chunks rather than
// their logical TotalSize. Falls back to logical size when the stat backend is
// unavailable.

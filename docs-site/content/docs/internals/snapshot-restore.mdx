...
description: How a sandbox boots in 49 ms — the path from snapshot file on disk to sshd accepting connections.
---

PandaStack doesn't cold-boot Linux every time you ask for a sandbox. We boot Linux **once** at template-bake time, write the entire VM state (memory + CPU registers + device state) to disk, and on every subsequent create we `mmap` that state into a fresh Firecracker process and let it execute the very next instruction. That's how p50 is **49 ms** and p99 is **64 ms** on bare-metal Intel Cascade Lake.
...
t=0       create request enters agent
   ...
   │
   └── return sandbox JSON  (boot_ms ≈ 49)
```

...
There is no volume count or storage quota — the open-source edition is unmetered. A hard **64 GiB per-volume ceiling** applies, and a host with no free space returns 507. See [Volumes](/docs/concepts/volumes).
...
| `BadRequestError`      | 4xx other — bad payload, validation failure, rate-limit guard.      |

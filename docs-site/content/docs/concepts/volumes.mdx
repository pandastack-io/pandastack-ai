...
Backing layout: one file per volume at `<data-dir>/volumes/ws/<workspace>/<name>.ext4`. Maximum size **65,536 MB** (64 GiB) per volume. Names are workspace-scoped.
...
**Shared model weights for an LLM workspace.** Mount a 30 GB volume of `huggingface/transformers` weights read-only across every inference sandbox. Cold-boot a sandbox in ~50 ms; `transformers` finds the cache on `/mnt/models`, skips re-download, runs in seconds.
...
There is **no volume count or storage quota** in the open-source edition — it
ships unmetered and uncapped. The only limits are physical:

- A hard ceiling of **64 GiB per volume**. Requests above it are rejected.
- If the target host has no free space left for the requested size, create returns **HTTP 507** — retry (the scheduler picks again) or ask for a smaller size.
- A single VM can carry **14** volumes (24 virtio slots minus the rootfs and reserved devices).

Beyond that, you are bounded only by the disk on your agent hosts.

## Limits

| Limit               | Value                                          |
| ------------------- | ---------------------------------------------- |
| Max size per volume | 64 GiB (hard ceiling)                          |
| Max volumes / sandbox | 14 (24 minus rootfs minus reserved slots)    |
| Max volumes / workspace | Unlimited — bounded by host disk            |

...
The current SDKs ship `Sandbox.run_code()` / `Sandbox.runCode()` (one-shot only). Stateful session bindings are tracked in [GitHub issues](https://github.com/pandastack-io/pandastack-ai/issues) — for now, call the REST endpoints directly.

---
title: Performance
description: How PandaStack hits 49 ms on every create via snapshot restore, and what to expect in your own benchmarks.
---
...
Last refresh: **2026-06-17**, snapshot-natid mode on XFS+reflink, `base` template, `api.pandastack.ai` → `us-central1`.
...
There is **no warm pool of idle VMs** — every create takes the same 49 ms path: restore the template's baked snapshot into a pre-allocated NAT slot.
...
| ----------------- | ----------------------------------------------------- | ----------------- |
| `snapshot-natid`  | Fresh restore from a memory snapshot into a free NAT slot. | 45–65 ms       |
...
2. **Seeing `cold` in `info.boot_mode`?** The template hadn't been baked on that host yet. The first boot captures the snapshot; subsequent creates drop to ~50 ms. For many sandboxes from one starting state, use `fork` instead.
3. **Template too big?** Cold-first-boot of a >5 GB rootfs takes 8–12 s. Once the snapshot exists, subsequent boots drop to ~50 ms.

...
If your sandbox runs a server on a port, that port is reachable instantly. A process started by the `pandastack-autostart` systemd unit is listening by the time the sandbox reports `running`, so the URL works immediately.
...
- Each preview hostname carries the same per-token abuse guard as the rest of the API (1 req/s burst 10 by default), configurable on the API. It is a runaway-loop guard, not a usage quota.

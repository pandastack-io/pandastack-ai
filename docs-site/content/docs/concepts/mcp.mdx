...
Looking for workspace-level tools (create/delete sandboxes, databases) behind a single URL? That's the [workspace MCP server](/docs/concepts/mcp-server) at `/mcp` — this page covers the per-sandbox endpoint.
...
mcp call \
  ...
  --header "Authorization: Bearer $PANDASTACK_API_KEY" \
...
- Every `tools/call shell` is one exec call, and shows up in the audit log as one.

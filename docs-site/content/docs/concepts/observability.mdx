...
Four pages drive the entire UX, all backed by the above:
...
**Capacity attribution by team.** Tag sandboxes with `metadata.team=…` at create, then group ClickHouse rollups by metadata to see which team is consuming which share of the fleet's vCPU-hours.
